...
const baseApiUrl = async () => {
  try {
    const base = await axios.get(
      `https://raw.githubusercontent.com/cyber-ullash/cyber-ullash/main/UllashApi.json`
    );
    return base.data.api;
  } catch (err) {
    throw new Error("Failed to fetch base API URL: " + err.message);
  }
};
...
module.exports.run = async ({ api, args, event }) => {
  ...
  try {
    ...
    for (const info of result) {
      thumbnails.push(diptoSt(info.thumbnail, 'photo' + i + '.jpg'));
      msg += `${i++}. ${info.title}\nTime: ${info.time}\nChannel: ${info.channel.name}\n\n`;
    }

    api.sendMessage({
      body: msg + "Reply to this message with a number you want to listen",
...
module.exports.handleReply = async ({ event, api, handleReply }) => {
  try {
    ...
    if (!isNaN(choice) && choice <= result.length && choice > 0) {
      ...
      api.sendMessage("Invalid choice. Please enter a number between 1 and " + result.length, event.threadID, event.messageID);
    }

  } catch (error) {
    console.log(error);
    api.sendMessage("⭕ Sorry, audio size may exceed 25MB or another error occurred", event.threadID, event.messageID);
  }
};
